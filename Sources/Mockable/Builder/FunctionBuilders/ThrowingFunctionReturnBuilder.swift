...
> {
    ...
    /// Registers a result type that automatically emits a value when successful, or throws when a failure
    ///
    /// - Parameter result: The result type wrapping a value or error.
    /// - Returns:  The parent builder, used for chaining additional specifications.
    @discardableResult
    public func willRespond(_ result: Result<ReturnType, ErrorType>) -> ParentBuilder {
        switch result {
